...
export default {
  ...
  methods: {
    async addShopcar() {
      try {
        let result = await this.$store.dispatch("addOrUpdataShopCart", {
          skuId: this.$route.params.skuId,
          skuNum: this.skuNum,
        });
        // this.$router.push(`/addcartsuccess?${{skuInfo:this.skuInfo,skuNum:this.skuNum}}`)
        //用本地存储（浏览器存储功能：本地存储和会话存储）存储skuInfo来避免用路由传参传递对象
        //用会话存储，会话结束就销毁
        //本地存储和会话存储存储的都是字符串,所以要将对象转为字符串存储，在提取时候转为对象
    ...
    changeSkuNum(event) {
      //修改商品详情页的需要购买的个数
      let skunum = event.target.value * 1; //排除非法输入
